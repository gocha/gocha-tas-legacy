...
function gettraceline(cpuname)
	...
	local decode  = function(offset_type, addr)
		local r = 0

		if offset_type == OPTYPE_DP then
			r = bit.band(regs.d + bit.band(addr, 0xffff), 0xffff)
		elseif offset_type == OPTYPE_DPX then
			r = bit.band((regs.d + regs.x + bit.band(addr, 0xffff)), 0xffff)
		elseif offset_type == OPTYPE_DPY then
			r = bit.band((regs.d + regs.y + bit.band(addr, 0xffff)), 0xffff)
		elseif offset_type == OPTYPE_IDP then
			addr = bit.band((regs.d + bit.band(addr, 0xffff)), 0xffff)
			r = bit.lshift(regs.db, 16) + memory.readword(addr)
		elseif offset_type == OPTYPE_IDPX then
			addr = bit.band((regs.d + regs.x + bit.band(addr, 0xffff)), 0xffff)
			r = bit.lshift(regs.db, 16) + memory.readword(addr)
		elseif offset_type == OPTYPE_IDPY then
			addr = bit.band((regs.d + bit.band(addr, 0xffff)), 0xffff)
			r = bit.lshift(regs.db, 16) + memory.readword(addr) + regs.y
		elseif offset_type == OPTYPE_ILDP then
			addr = bit.band(regs.d + bit.band(addr, 0xffff), 0xffff)
			r = readwordbyte(addr)
		elseif offset_type == OPTYPE_ILDPY then
			addr = bit.band((regs.d + bit.band(addr, 0xffff)), 0xffff)
			r = readwordbyte(addr) + regs.y
		elseif offset_type == OPTYPE_ADDR then
			r = bit.lshift(regs.db, 16) + bit.band(addr, 0xffff)
		elseif offset_type == OPTYPE_ADDR_PC then
			r = bit.lshift(regs.pb, 16) + bit.band(addr, 0xffff)
		elseif offset_type == OPTYPE_ADDRX then
			r = bit.lshift(regs.db, 16) + bit.band(addr, 0xffff) + regs.x
		elseif offset_type == OPTYPE_ADDRY then
			r = bit.lshift(regs.db, 16) + bit.band(addr, 0xffff) + regs.y
		elseif offset_type == OPTYPE_IADDR_PC then
			r = bit.lshift(regs.pb, 16) + bit.band(addr, 0xffff)
		elseif offset_type == OPTYPE_IADDRX then
			r = bit.lshift(regs.pb, 16) + bit.band((addr + regs.x), 0xffff)
		elseif offset_type == OPTYPE_ILADDR then
			r = addr
		elseif offset_type == OPTYPE_LONG then
			r = addr
		elseif offset_type == OPTYPE_LONGX then
			r = (addr + regs.x)
		elseif offset_type == OPTYPE_SR then
			r = bit.band((regs.s + bit.band(addr, 0xff)), 0xffff)
		elseif offset_type == OPTYPE_ISRY then
			addr = bit.band((regs.s + bit.band(addr, 0xff)), 0xffff)
			r = bit.lshift(regs.db, 16) + memory.readword(addr) + regs.y
		elseif offset_type == OPTYPE_RELB then
			r = bit.lshift(regs.pb, 16) + bit.band((regs.pc + 2), 0xffff) + toint8(addr)
		elseif offset_type == OPTYPE_RELW then
			r = bit.lshift(regs.pb, 16) + bit.band((regs.pc + 3), 0xffff) + toint16(addr)
		end

		return bit.band(r, 0xffffff)
	end
