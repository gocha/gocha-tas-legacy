...
function adjustFrameState(e)
	...
	local players = { e.player }
	for i, player in ipairs(players) do
		player.blink = false -- boolnum(player.blink)
		player.visible = true -- ((math.floor(player.visual / 0x80) % 2) == 0)
...
--              provide enough
-- Data: frame -> x, y, region, roomx, roomy, pose, albus
...
function draw_ghost_gfx(ghost,frame,index,logonly)
	...
	if emudata[1].mode ~= 0 then return end
	...
	for i, player in ipairs(players) do
		local put = function(x, y)
			if player.visible then
				...
				local opacity = ghost.opacity * math.min(1.0, 1.0 - math.floor(math.abs(emudata[3].fade)/0x1000)/16.0) * (player.blink and 0.5 or 1.0)
				if not reverse then
					if logonly then
